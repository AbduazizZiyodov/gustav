...
def test(capfd: CaptureFixture, file_path: Path) -> None:
    expected, expected_exit_code = extract_expected(file_path)
    extracted_result, source_code, returncode = run_file(capfd, file_path)

    assert extracted_result == expected, f"[FAILED] '{file_path}'\n == source code ==\n{source_code}\n== /source code =="

    if expected_exit_code is not None:
        assert returncode == expected_exit_code, f"[FAILED] '{file_path} on exit/return code assertion"